    ...
    if not token or token == 'guest':
        ...
        connected_clients[sid] = {
            ...
            'client_info': {
                ...
                'transport': request.environ.get('wsgi.url_scheme', 'unknown') if hasattr(request, 'environ') else 'unknown'
            }
        }
        return True
...
def handle_join(data):
    ...
    try:
        ...
        logger.info(f"JOIN REQUEST - User {visitor_id or client['user_id']} joining room {room_id}")
        
        # More lenient ID validation - allow if supplied ID is None/empty or matches authenticated ID
        if visitor_id and visitor_id != client['user_id'] and not client['user_id'].startswith('guest-'):
            logger.warning(f"JOIN WARNING - User ID mismatch: expected {client['user_id']}, got {visitor_id}")
            # Allow join anyway in development mode
            if os.environ.get('FLASK_ENV') == 'development':
                logger.warning(f"JOIN WARNING - Allowing despite ID mismatch (development mode)")
            else:
...
def handle_message(data):
    ...
    try:
        ...
        logger.info(f"MESSAGE RECEIVED - Room: {room_id}, User: {client['user_id']}, Content: {message[:30]}...")
        ...
        try:
            ...
            logger.info(f"Storing message in database: room={room_id}, message_id={message_id}")
            ...
            while retry_count < max_retries and not success:
                try:
                    ...
                    if response.status_code in (200, 201):
                        logger.info(f"STORAGE SUCCESS - Message stored in database: room={room_id}, message_id={message_id}")
                        success = True
                    else:
                        logger.error(f"STORAGE ERROR - Failed to store message in database. Status: {response.status_code}")
                        retry_count += 1
                        # Exponential backoff
                        time.sleep(0.5 * retry_count) 
                except requests.RequestException as e:
                    logger.error(f"STORAGE ERROR - Error storing message in database: {str(e)}")
                    retry_count += 1
                    # Exponential backoff
                    time.sleep(0.5 * retry_count)
            
            if not success:
                logger.error(f"CRITICAL - Failed to store message after {max_retries} retries: room={room_id}")
        ...
        try:
            ...
            room_clients = [cid for cid, cli in connected_clients.items() if room_id in cli['rooms']]
            logger.info(f"ROOM CLIENTS: Room {room_id} has {len(room_clients)} connected clients")
...
def handle_user_active(data):
    ...
    try:
        ...
        logger.info(f"USER_ACTIVE - User {client['user_id']} is active in room {room_id}")
    except Exception as e:
...
def handle_subscribe(data):
    ...
    try:
        ...
        logger.info(f"SUBSCRIBE SUCCESS - User {client['user_id']} subscribed to room {room}")
...
if __name__ == '__main__':
    ...
    socketio.run(app, host='0.0.0.0', port=5006, debug=(os.environ.get('DEBUG', 'false').lower() == 'true')) 