...
pkill -f "python3 api_gateway/app.py" || true
pkill -f "python3 realtime_service/app" || true
sleep 1
...
# Create a fixed version of the realtime service
cat > realtime_service/app_fixed_room.py << 'EOF'
"""
...
echo "Starting fixed realtime service on 0.0.0.0:5506..."
python3 realtime_service/app_fixed_room.py > realtime_service_final.log 2>&1 &
REALTIME_PID=$!
echo "Fixed realtime service started with PID: $REALTIME_PID"
sleep 2

echo "Starting API gateway with external host ${EXTERNAL_HOST}..."
cd /home/aqniyet/abdre_api
